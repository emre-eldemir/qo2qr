<?php $layout = 'admin'; $title = 'Form Assignments'; $pageTitle = 'Form-Table Assignments'; ?>
<?php $sidebarMenu = [
    ...
    ['url' => '/customer/form-assignments', 'icon' => 'link-45deg', 'label' => 'Form Assignments', 'active' => true],
...
<div class="row g-4">
    <div class="col-lg-5">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white"><h5 class="mb-0">Assign Form to Table</h5></div>
            <div class="card-body">
                <form method="POST" action="<?= url('/customer/form-assignments/store') ?>">
                    <?= CSRF::field() ?>
                    <div class="mb-3">
                        ...
                        <select name="form_id" class="form-select" required>
                            ...
                            <?php foreach ($forms as $f): ?>
                                <option value="<?= h((string) $f['id']) ?>"><?= h($f['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mb-3">
    ...
    <div class="col-lg-7">
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white"><h5 class="mb-0">Current Assignments</h5></div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead class="table-light"><tr><th>Form</th><th>Table</th><th>Since</th><th>Action</th></tr></thead>
                        <tbody>
                            ...
                            <?php else: ?>
                                <?php foreach ($assignments as $a): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <form method="POST" action="<?= url('/customer/form-assignments/delete/' . (int) $a['id']) ?>" class="d-inline" onsubmit="return confirm('Remove?')">
                                                <?= CSRF::field() ?>
